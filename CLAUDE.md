...
| Fájl | Szerep |
|------|--------|
| `radar.py` | Minden logika egyben: `FORRASOK` (10 forrás), letöltés (requests), linkkigyűjtés (BeautifulSoup), `KULCSSZAVAK`-szűrés, URL-normalizálás, NKA-speciális kezelés, allapot-diff, `report.md` írás, `GITHUB_OUTPUT`-ba `new_count` + `first_run` |
...
| `allapot.json` | Kulcs: normalizált URL vagy `nka-kollegium:<név>`; érték: első észlelés dátuma. **A bot commitolja naponta** — kézzel ne szerkeszd, munka előtt mindig `git pull`! |
...
- `palyazat.gov.hu` JS-alapú, géppel olvashatatlan → a `palyazatok.org/kkv-palyazatok/` az aggregátor-proxy (kft-s források); a hivatalos részletek a palyazat.gov.hu-n.
